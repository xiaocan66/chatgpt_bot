...
func (t *TelegramBot) Init(cfg *cfg.Config) error {
	...
	t.channelName = cfg.BotConfig.Telegram.TelegramChannelName
	t.groupName = cfg.BotConfig.Telegram.TelegramGroupName
	t.limitPrivate = cfg.BotConfig.ShouldLimitPrivate
	t.limitGroup = cfg.BotConfig.ShouldLimitGroup
	t.logChat = cfg.BotConfig.Telegram.LogChannelID
	...
	t.session = &sync.Map{}
	bot, err := tgbotapi.NewBotAPI(cfg.BotConfig.Telegram.TelegramBotToken)
	if err != nil {
		return err
	}
	t.tgBot = bot
